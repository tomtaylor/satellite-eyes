...
- (instancetype)init
{
    ...
    if (self) {
        ...
        nw_path_monitor_set_update_handler(pathMonitor, ^(nw_path_t path) {
            BOOL wasSatisfied = self->networkSatisfied;
            self->networkSatisfied = (nw_path_get_status(path) == nw_path_status_satisfied);

            if (self->networkSatisfied && !wasSatisfied) {
                [self updateMap];
            } else if (!self->networkSatisfied && wasSatisfied) {
                [self restartMap];
            }
        });
